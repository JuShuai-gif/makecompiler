...
static intptr_t _risc_color_select(graph_node_t* node, vector_t* colors, function_t* f)
{
...
static int _risc_color_del(vector_t* colors, intptr_t color, function_t* f)
{
	int i = 0;
...
static int _risc_kcolor_check(graph_t* graph)
{
	int i;
...
static int _risc_kcolor_delete(graph_t* graph, int k, vector_t* deleted_nodes)
{
...
static int _risc_kcolor_fill(graph_t* graph, int k, vector_t* colors,
		vector_t* deleted_nodes, function_t* f)
{
...
static int _risc_kcolor_find_not_neighbor(graph_t* graph, int k, graph_node_t** pp0, graph_node_t** pp1)
{
...
static graph_node_t* _risc_max_neighbors(graph_t* graph)
{
	...
	if (rn->dag_node->var->w)
		logi("max_neighbors: %d, %s\n", max, rn->dag_node->var->w->text->data);
	else
		logi("max_neighbors: %d, v_%p\n", max, rn->dag_node->var);

	return node_max;
}

static void _risc_kcolor_process_conflict(graph_t* graph, function_t* f)
{
	int i;
	int j;
	int ret;
...
static int _risc_graph_kcolor(graph_t* graph, int k, vector_t* colors, function_t* f)
{
	int ret = -1;
	vector_t* colors2 = NULL;
	...
	if (0 == _risc_kcolor_find_not_neighbor(graph, k, &node0, &node1)) {
		...
		ret = risc_graph_kcolor(graph, k - 1, colors2, f);
		if (ret < 0)
			goto error;
...
overflow:
		...
		ret = risc_graph_kcolor(graph, k, colors, f);
		if (ret < 0)
			goto error;
