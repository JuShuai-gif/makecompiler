...
int main()
{
	...
	if (elf_open(&elf, "x64", "./1.elf") < 0) {
		printf("%s(),%d, error\n", __func__, __LINE__);
		return -1;
	}

	elf_section_t*	section = calloc(1, sizeof(elf_section_t));
	assert(section);
	elf_sym_t*		sym		= calloc(1, sizeof(elf_sym_t));
	assert(sym);
	elf_rela_t*		rela		= calloc(1, sizeof(elf_rela_t));
	assert(rela);
