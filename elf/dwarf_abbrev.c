...
{
	dwarf_abbrev_declaration_t* d = calloc(1, sizeof(dwarf_abbrev_declaration_t));
	if (!d)
		return NULL;

	d->attributes = vector_alloc();
	if (!d->attributes) {
		free(d);
		return NULL;
	}
...
void dwarf_abbrev_declaration_free(dwarf_abbrev_declaration_t* d)
{
	if (d) {
		if (d->attributes) {
			vector_clear(d->attributes, (void (*)(void*)) free);
			vector_free(d->attributes);
		}

		if (d->childs)
			vector_free(d->childs);
...
int dwarf_abbrev_decode(vector_t* abbrev_results, const char*   debug_abbrev, size_t debug_abbrev_size)
{
	...
	dwarf_abbrev_declaration_t* parent = NULL;
	...
	while (i < debug_abbrev_size) {
		...
		ret = vector_add(abbrev_results, d);
		if (ret < 0) {
			dwarf_abbrev_declaration_free(d);
			return ret;
		}
		...
		len     = 0;
		d->code = leb128_decode_uint32(debug_abbrev + i, &len);
		assert(len > 0);
		i += len;
		...
		len    = 0;
		d->tag = leb128_decode_uint32(debug_abbrev + i, &len);
		assert(len > 0);
		i += len;
		assert(i < debug_abbrev_size);
		...
		loge("d: %p, d->parent: %p, d->code: %u, d->tag: %u, d->has_children: %u\n",
		...
		do {
			...
			len = 0;
			attr->name = leb128_decode_uint32(debug_abbrev + i, &len);
			assert(len > 0);
			i += len;
			assert(i < debug_abbrev_size);

			len = 0;
			attr->form = leb128_decode_uint32(debug_abbrev + i, &len);
			assert(len > 0);
			i += len;
			assert(i < debug_abbrev_size);

			logd("attr: %p, attr->name: %u, attr->form: %u\n",
				attr, attr->name, attr->form);

			int ret = vector_add(d->attributes, attr);
			if (ret < 0) {
				free(attr);
				return ret;
			}
		...
		if (parent) {
			...
			ret = vector_add(parent->childs, d);
			if (ret < 0)
				return ret;
		}

		if (d->has_children) {
			d->childs = vector_alloc();
			if (!d->childs)
				return -ENOMEM;
...
static int _dwarf_abbrev_encode(dwarf_abbrev_declaration_t* d, string_t* debug_abbrev)
{
...
#define DWARF_ABBREV_FILL(str, buf, len) \
	...
	len = leb128_encode_uint32(d->code, buf, sizeof(buf));
	assert(len > 0);
	DWARF_ABBREV_FILL(debug_abbrev, buf, len);

	len = leb128_encode_uint32(d->tag, buf, sizeof(buf));
	assert(len > 0);
	DWARF_ABBREV_FILL(debug_abbrev, buf, len);
	...
	for (j = 0; j < d->attributes->size; j++) {
		...
		len = leb128_encode_uint32(attr->name, buf, sizeof(buf));
		assert(len > 0);
		DWARF_ABBREV_FILL(debug_abbrev, buf, len);

		len = leb128_encode_uint32(attr->form, buf, sizeof(buf));
		assert(len > 0);
		DWARF_ABBREV_FILL(debug_abbrev, buf, len);
	}
...
int dwarf_abbrev_encode(vector_t* abbrev_results, string_t* debug_abbrev)
{
	...
	for (i = 0; i < abbrev_results->size; i++) {
		...
		if (d->code)
			logd("d->code: %u, d->tag: %s\n", d->code, dwarf_find_tag(d->tag));
	}
...
static void _dwarf_abbrev_print(dwarf_abbrev_declaration_t* d)
{
	dwarf_abbrev_declaration_t* d2;
	dwarf_abbrev_attribute_t*   attr;
	...
	printf("code: %u  %s  has_children: %u\n", d->code, dwarf_find_tag(d->tag), d->has_children);
	...
	for (j = 0; j < d->attributes->size; j++) {
		...
		if (0 == attr->name)
			...
			int len = printf("%s ", dwarf_find_attribute(attr->name));
			len = 30 - len;
