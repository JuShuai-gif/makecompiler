...
static int _arm32_elf_add_interp(elf_native_t* arm32, elf_section_t** ps)
{
	...
	int ret = vector_add(arm32->sections, s);
	if (ret < 0) {
		string_free(s->name);
		free(s->data);
		free(s);
...
//	_arm32_elf_add_gnu_version_r(arm32, &arm32->gnu_version_r);
	...
	for (i = 0; i < arm32->dyn_needs->size; i++) {
		string_t* needed = arm32->dyn_needs->data[i];
...
//	arm32->gnu_version_r->sh.sh_addr = rx_base + arm32->gnu_version_r->offset;
	...
	for (i = 0; i < arm32->dynsyms->size; i++) {
		...
		*got_plt = arm32->plt->sh.sh_addr;
		...
		logd("i: %d, got_addr: %#lx, plt_addr: %#lx, offset: %d, %#x\n", i, got_addr, plt_addr, offset, offset);
...
int __arm32_elf_write_phdr(elf_context_t* elf, uint64_t rx_base, uint64_t offset, uint32_t nb_phdrs)
{
...
int __arm32_elf_write_interp(elf_context_t* elf, uint64_t rx_base, uint64_t offset, uint64_t len)
{
...
int __arm32_elf_write_text(elf_context_t* elf, uint64_t rx_base, uint64_t offset, uint64_t len)
{
...
int __arm32_elf_write_rodata(elf_context_t* elf, uint64_t r_base, uint64_t offset, uint64_t len)
{
...
int __arm32_elf_write_data(elf_context_t* elf, uint64_t rw_base, uint64_t offset, uint64_t len)
{
...
int __arm32_elf_write_dynamic(elf_context_t* elf, uint64_t rw_base, uint64_t offset, uint64_t len)
{
