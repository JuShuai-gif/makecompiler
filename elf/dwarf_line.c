...
{
	dwarf_line_machine_t* lm;

	lm = calloc(1, sizeof(dwarf_line_machine_t));
	if (!lm)
		return NULL;

	lm->prologue = calloc(1, sizeof(dwarf_line_prologue_t));
	if (!lm->prologue) {
		free(lm);
		return NULL;
	}
	...
	if (lm) {
		if (lm->prologue) {
			...
			if (lm->prologue->file_names) {
				vector_clear(lm->prologue->file_names, ( void (*)(void*) )dwarf_line_filename_free);
				vector_free(lm->prologue->file_names);
			}
...
static dwarf_ubyte_t standard_opcode_lengths[13] = {
	0, 1, 1, 1, 1,
	0, 0, 0, 1, 0,
	0, 1, 0
};
...
int dwarf_line_machine_fill(dwarf_line_machine_t* lm, vector_t* file_names)
{
	...
	lm->prologue->standard_opcode_lengths = calloc(lm->prologue->opcode_base, sizeof(dwarf_ubyte_t));
	if (!lm->prologue->standard_opcode_lengths)
		return -ENOMEM;

	memcpy(lm->prologue->standard_opcode_lengths, standard_opcode_lengths, 13 * sizeof(dwarf_ubyte_t));

	lm->prologue->include_directories = NULL;

	lm->prologue->file_names = vector_alloc();
	if (!lm->prologue->file_names)
		return -ENOMEM;

	dwarf_line_filename_t* f;
	int i;

	for (i = 0; i  < file_names->size; i++) {

		string_t* fname = file_names->data[i];

		f = calloc(1, sizeof(dwarf_line_filename_t));
		if (!f)
			return -ENOMEM;
		...
		if (vector_add(lm->prologue->file_names, f) < 0) {
			free(f->name);
			free(f);
			return -ENOMEM;
		}
...
static inline void _decode_special_opcode(dwarf_line_prologue_t* prologue, dwarf_ubyte_t opcode, dwarf_uword_t* paddress_advance, dwarf_sword_t* pline_advance)
{
	opcode -= prologue->opcode_base;
	*paddress_advance = opcode / prologue->line_range;
	*pline_advance    = opcode % prologue->line_range + (dwarf_sword_t)prologue->line_base;
}

int dwarf_line_decode(dwarf_line_machine_t* lm, vector_t* line_results, const char*   debug_line, size_t debug_line_size)
{
	if (!lm || !line_results || !debug_line || debug_line_size < sizeof(dwarf_uword_t)) {
		loge("\n");
		return -EINVAL;
	}

	lm->prologue->total_length = *(dwarf_uword_t*)debug_line;

	if (sizeof(dwarf_uword_t) + lm->prologue->total_length != debug_line_size) {
		loge("\n");
		return -EINVAL;
	}
	...
	lm->prologue->version = *(dwarf_uhalf_t*)(debug_line + i);
	i += sizeof(dwarf_uhalf_t);

	lm->prologue->prologue_length = *(dwarf_uword_t*)(debug_line + i);
	i += sizeof(dwarf_uword_t);

	lm->prologue->minimum_instruction_length = *(dwarf_ubyte_t*)(debug_line + i);
	i += sizeof(dwarf_ubyte_t);

	lm->prologue->default_is_stmt = *(dwarf_ubyte_t*)(debug_line + i);
	i += sizeof(dwarf_ubyte_t);

	lm->prologue->line_base   = *(dwarf_sbyte_t*)(debug_line + i);
	i += sizeof(dwarf_sbyte_t);

	lm->prologue->line_range  = *(dwarf_ubyte_t*)(debug_line + i);
	i += sizeof(dwarf_ubyte_t);

	lm->prologue->opcode_base = *(dwarf_ubyte_t*)(debug_line + i);
	i += sizeof(dwarf_ubyte_t);
	...
	lm->prologue->standard_opcode_lengths = malloc(n * sizeof(dwarf_ubyte_t));
	if (!lm->prologue->standard_opcode_lengths)
		return -ENOMEM;
	...
	while (i < debug_line_size
		...
		size_t   len = 0;
		uint32_t val = leb128_decode_uint32(debug_line + i, &len);
		assert(len > 0);
	...
	if (!lm->prologue->file_names) {
		...
		vector_clear(lm->prologue->file_names, (void (*)(void*))free);
	}
	...
	while (i < debug_line_size) {
		...
		dwarf_line_filename_t* filename = calloc(1, sizeof(dwarf_line_filename_t));
		if (!filename)
			return -ENOMEM;
		...
		len = 0;
		val = leb128_decode_uint32(debug_line + i, &len);
		assert(len > 0);
		...
		int ret = vector_add(lm->prologue->file_names, filename);
		if (ret < 0) {
			loge("\n");
			return ret;
		}
	...
	while (i < debug_line_size) {
		...
		dwarf_line_result_t* result = NULL;
		int ret;

		dwarf_ubyte_t opcode;
		dwarf_uword_t address_advance;
		dwarf_sword_t line_advance;
		dwarf_uhalf_t uhalf;
		...
		if (opcode >= lm->prologue->opcode_base) {
			...
			result = calloc(1, sizeof(dwarf_line_result_t));
			if (!result)
				return -ENOMEM;
			...
			ret = vector_add(line_results, result);
			if (ret < 0)
				return ret;
			...
			switch (opcode) {
				case DW_LNS_copy:
					...
					logd("opcode: %u, address: %#lx, line: %u\n", opcode, lm->address, lm->line);
				...
				case DW_LNS_advance_pc:
					len = 0;
					address_advance  = leb128_decode_uint32(debug_line + i, &len);
					address_advance *= lm->prologue->minimum_instruction_length;
					lm->address     += address_advance;

					logd("opcode: %u, address: %#lx, line: %u, address_advance: %u\n",
				...
				case DW_LNS_advance_line:
					len = 0;
					line_advance  = leb128_decode_int32(debug_line + i, &len);
					lm->line     += line_advance;
				...
				case DW_LNS_set_file:
					len = 0;
					lm->file = leb128_decode_uint32(debug_line + i, &len);
				...
				case DW_LNS_set_column:
					len = 0;
					lm->column = leb128_decode_uint32(debug_line + i, &len);
			...
			size_t len = 0;
			size_t num = leb128_decode_uint32(debug_line + i, &len);
			assert(len > 0);
			assert(num > 0);
			i += len;
			...
			if (DW_LNE_end_sequence == opcode) {
				...
				j++;
				len = 0;
				uint32_t dir_index = leb128_decode_uint32(debug_line + j, &len);
				assert(len > 0);
				j += len;

				len = 0;
				uint32_t time_modified = leb128_decode_uint32(debug_line + j, &len);
				assert(len > 0);
				j += len;

				len = 0;
				uint32_t file_length = leb128_decode_uint32(debug_line + j, &len);
				assert(len > 0);
				j += len;

				loge("dir_index:     %u\n", dir_index);
				loge("time_modified: %u\n", time_modified);
				loge("file_length:   %u\n", file_length);
...
#define DWARF_DEBUG_LINE_FILL(data) \
	do { \
		int ret = string_cat_cstr_len(debug_line, (char*)&(data), sizeof(data)); \
		if (ret < 0) { \
			loge("\n"); \
			return ret; \
		} \
...
#define DWARF_DEBUG_LINE_FILL2(buf, len) \
	do { \
		int ret = string_cat_cstr_len(debug_line, (char*)buf, len); \
		if (ret < 0) { \
			loge("\n"); \
			return ret; \
		} \
	...
	for (i = 0; i < lm->prologue->opcode_base - 1; i++) {
		...
		size_t len = leb128_encode_uint32(val, buf, sizeof(buf));
		assert(len > 0);

		DWARF_DEBUG_LINE_FILL2(buf, len);
	};

	dwarf_ubyte_t include_directories = 0;
	DWARF_DEBUG_LINE_FILL(include_directories);

	for (i = 0; i < lm->prologue->file_names->size; i++) {

		dwarf_line_filename_t* filename = lm->prologue->file_names->data[i];
		...
		len = leb128_encode_uint32(filename->dir_index, buf, sizeof(buf));
		assert(len > 0);
		DWARF_DEBUG_LINE_FILL2(buf, len);

		len = leb128_encode_uint32(filename->time_modified, buf, sizeof(buf));
		assert(len > 0);
		DWARF_DEBUG_LINE_FILL2(buf, len);

		len = leb128_encode_uint32(filename->length, buf, sizeof(buf));
		assert(len > 0);
		DWARF_DEBUG_LINE_FILL2(buf, len);
	}

	dwarf_ubyte_t end_file_names = 0;
	...
	logd("lm->prologue->prologue_length: %u\n\n", lm->prologue->prologue_length);
	...
	for (i = 0; i < line_results->size; i++) {
		...
		dwarf_line_result_t* result = line_results->data[i];

		dwarf_sword_t line_advance;
		dwarf_uword_t address_advance;
		dwarf_uword_t opcode;

		logd("i: %d, line: %d, address: %#lx\n", i, result->line, result->address);
		...
		if (0 == i) {
			...
			dwarf_ubyte_t extended_flag   = 0;
			dwarf_ubyte_t extended_opcode = DW_LNE_set_address;

			len = leb128_encode_uint32(1 + sizeof(uintptr_t), buf, sizeof(buf));
			assert(len > 0);
			...
			rela->name = string_cstr(".text");
			if (!rela->name) {
				free(rela);
				return -ENOMEM;
			}

			if (vector_add(debug->line_relas, rela) < 0) {
		...
		string_t* file_name = debug->file_names->data[lm->file - 1];
		...
		if (strcmp(file_name->data, result->file_name->data)) {
			...
			len    = leb128_encode_uint32(lm->file, buf, sizeof(buf));
			assert(len > 0);
			DWARF_DEBUG_LINE_FILL2(&opcode, 1);
			DWARF_DEBUG_LINE_FILL2(buf,     len);
		}
		...
		logd("result->line: %d, lm->line: %d, line_advance: %d, address_advance: %u\n",
		...
		while (1) {
			if (line_advance - lm->prologue->line_base < lm->prologue->line_range
				...
				if (opcode <= 255) {
					...
					DWARF_DEBUG_LINE_FILL2(&opcode, 1);
					logd("result->line: %d, line_advance: %d, address_advance: %u, opcode: %d\n",
			...
			if (line_advance != 0) {
				...
				len    = leb128_encode_int32(line_advance, buf, sizeof(buf));
				assert(len > 0);
				DWARF_DEBUG_LINE_FILL2(&opcode, 1);
				DWARF_DEBUG_LINE_FILL2(buf,     len);
			...
			if (address_advance > 0) {
				...
				len    = leb128_encode_uint32(address_advance, buf, sizeof(buf));
				assert(len > 0);
				DWARF_DEBUG_LINE_FILL2(&opcode, 1);
				DWARF_DEBUG_LINE_FILL2(buf,     len);
		...
		if (result->end_sequence) {
			...
			dwarf_ubyte_t extended_flag   = 0;
			dwarf_ubyte_t extended_opcode = DW_LNE_end_sequence;

			len = leb128_encode_uint32(1, buf, sizeof(buf));
			assert(len > 0);
	...
	logd("lm->prologue->total_length: %u\n", lm->prologue->total_length);
