...
static int _arm64_elf_add_interp(elf_native_t* arm64, elf_section_t** ps)
{
	...
	int ret = vector_add(arm64->sections, s);
	if (ret < 0) {
		string_free(s->name);
		free(s->data);
		free(s);
...
static int _arm64_elf_add_gnu_version(elf_native_t* arm64, elf_section_t** ps)
{
	...
	s->name = string_cstr(".gnu.version");
	if (!s->name) {
		free(s);
		return -ENOMEM;
	}
...
static int _arm64_elf_add_gnu_version_r(elf_native_t* arm64, elf_section_t** ps)
{
	...
	s->name = string_cstr(".gnu.version_r");
	if (!s->name) {
		free(s);
		return -ENOMEM;
	}
...
//	_arm64_elf_add_gnu_version_r(arm64, &arm64->gnu_version_r);
	...
	for (i = 0; i < arm64->dynsyms->size; i++) {
		...
		logi("i: %d, st_value: %#lx\n", i, syms[i + 1].st_value);

		string_cat_cstr_len(str, xsym->name->data, xsym->name->len + 1);
	}
	...
	string_cat_cstr_len(str, "libc.so.6", strlen("libc.so.6") + 1);
	...
	string_cat_cstr_len(str, "GLIBC_2.4", strlen("GLIBC_2.4") + 1);
#endif
	...
	for (i = 0; i < arm64->dyn_needs->size; i++) {
		string_t* needed = arm64->dyn_needs->data[i];
		...
		string_cat_cstr_len(str, needed->data + prefix, needed->len - prefix + 1);
	}
...
//	arm64->gnu_version_r->sh.sh_addr = rx_base + arm64->gnu_version_r->offset;
	...
	logi("got_addr: %#lx, plt_addr: %#lx, offset: %d, %#x\n", got_addr, plt_addr, offset, offset);
	...
	for (i = 0; i < arm64->dynsyms->size; i++) {
		...
		*got_plt = arm64->plt->sh.sh_addr;
		...
		logi("i: %d, got_addr: %#lx, plt_addr: %#lx, offset: %d, %#x\n", i, got_addr, plt_addr, offset, offset);
...
int __arm64_elf_write_phdr(elf_context_t* elf, uint64_t rx_base, uint64_t offset, uint32_t nb_phdrs)
{
...
int __arm64_elf_write_interp(elf_context_t* elf, uint64_t rx_base, uint64_t offset, uint64_t len)
{
...
int __arm64_elf_write_text(elf_context_t* elf, uint64_t rx_base, uint64_t offset, uint64_t len)
{
...
int __arm64_elf_write_rodata(elf_context_t* elf, uint64_t r_base, uint64_t offset, uint64_t len)
{
...
int __arm64_elf_write_data(elf_context_t* elf, uint64_t rw_base, uint64_t offset, uint64_t len)
{
...
int __arm64_elf_write_dynamic(elf_context_t* elf, uint64_t rw_base, uint64_t offset, uint64_t len)
{
