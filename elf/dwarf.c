...
static dwarf_abbrev_attribute_t abbrev_cu[] =
{
...
static dwarf_abbrev_attribute_t abbrev_subprogram[] =
{
...
static dwarf_abbrev_attribute_t abbrev_base_type[] =
{
...
static dwarf_abbrev_attribute_t abbrev_var[] =
{
	{DW_AT_name,               DW_FORM_string},
	{DW_AT_decl_file,          DW_FORM_data1},
	{DW_AT_decl_line,          DW_FORM_data1},
...
static dwarf_abbrev_attribute_t abbrev_struct_type[] =
{
	{DW_AT_name,               DW_FORM_strp},
...
static dwarf_abbrev_attribute_t abbrev_member_var[] =
{
	{DW_AT_name,                 DW_FORM_string},
	{DW_AT_decl_file,            DW_FORM_data1},
	{DW_AT_decl_line,            DW_FORM_data1},
...
static dwarf_abbrev_attribute_t abbrev_pointer_type[] =
{
	{DW_AT_byte_size,          DW_FORM_data1},
	{DW_AT_type,               DW_FORM_ref4},
	{0,                        0},
...
static int __abbrev_add(vector_t* abbrevs)
{
	...
	d = dwarf_abbrev_declaration_alloc();
	if (!d)
		return -ENOMEM;

	if (vector_add(abbrevs, d) < 0) {
		free(d);
		return -ENOMEM;
	}

	if (vector_add(parent->childs, d) < 0)
		return -ENOMEM;
...
static int _abbrev_add_attributes(dwarf_abbrev_declaration_t* d, dwarf_abbrev_attribute_t* attributes, int nb_attributes)
{
	...
	for (i = 0; i < nb_attributes; i++) {
		...
		attr = malloc(sizeof(dwarf_abbrev_attribute_t));
		if (!attr)
			return -ENOMEM;

		if (vector_add(d->attributes, attr) < 0) {
			free(attr);
			return -ENOMEM;
		}
...
static int _abbrev_add(vector_t* abbrevs, dwarf_uword_t tag, dwarf_ubyte_t has_children, dwarf_abbrev_attribute_t* attributes, int nb_attributes)
{
	dwarf_abbrev_declaration_t* d;
...
int dwarf_abbrev_add_cu(vector_t* abbrevs)
{
	...
	return dwarf_abbrev_add_subprogram(abbrevs);
}
...
int dwarf_debug_encode(dwarf_t* debug)
{
	...
	vector_free(debug->infos);
	debug->infos = infos;
	infos        = NULL;

	int ret = dwarf_abbrev_encode(debug->abbrevs, debug->debug_abbrev);
	if (ret < 0) {
		loge("\n");
		return ret;
	}

	dwarf_info_header_t  header;
	...
	ret = dwarf_info_encode(debug, &header);
	if (ret < 0) {
		loge("\n");
		return ret;
	}
	...
	ret = dwarf_line_machine_fill(lm, debug->file_names);
	if (ret < 0)
		return ret;

	ret = dwarf_line_encode(debug, lm, debug->lines, debug->debug_line);
	dwarf_line_machine_free(lm);
	if (ret < 0)
		return ret;
	...
	n &= 0x7;
	if (n) {
		ret = string_fill_zero(debug->str, 8 - n);
		if (ret < 0)
			return ret;
	}
...
{
	dwarf_t* debug = calloc(1, sizeof(dwarf_t));
	if (!debug)
		return NULL;

	debug->base_types = vector_alloc();
	if (!debug->base_types) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->struct_types = vector_alloc();
	if (!debug->struct_types) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->lines = vector_alloc();
	if (!debug->lines) {
		free(debug);
		return NULL;
	}

	debug->infos = vector_alloc();
	if (!debug->infos) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->abbrevs = vector_alloc();
	if (!debug->abbrevs) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->str = string_alloc();
	if (!debug->str) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->debug_line = string_alloc();
	if (!debug->debug_line) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->debug_info = string_alloc();
	if (!debug->debug_info) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->debug_abbrev = string_alloc();
	if (!debug->debug_abbrev) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->line_relas = vector_alloc();
	if (!debug->line_relas) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->info_relas = vector_alloc();
	if (!debug->info_relas) {
		dwarf_debug_free(debug);
		return NULL;
	}

	debug->file_names = vector_alloc();
	if (!debug->file_names) {
		dwarf_debug_free(debug);
		return NULL;
	}

	if (dwarf_abbrev_add_cu(debug->abbrevs) < 0) {
		dwarf_debug_free(debug);
...
void dwarf_debug_free (dwarf_t* debug)
{
	if (debug) {
		if (debug->base_types)
			vector_free(debug->base_types);

		if (debug->struct_types)
			vector_free(debug->struct_types);

		if (debug->lines)
			vector_free(debug->lines);

		if (debug->infos)
			vector_free(debug->infos);

		if (debug->abbrevs)
			vector_free(debug->abbrevs);

		if (debug->str)
			string_free(debug->str);

		if (debug->debug_line)
			string_free(debug->debug_line);

		if (debug->debug_info)
			string_free(debug->debug_info);

		if (debug->debug_abbrev)
			string_free(debug->debug_abbrev);

		if (debug->line_relas)
			vector_free(debug->line_relas);

		if (debug->info_relas)
			vector_free(debug->info_relas);

		if (debug->file_names)
			vector_free(debug->file_names);
