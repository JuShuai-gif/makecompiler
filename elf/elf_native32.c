...
int elf32_open(elf_context_t* elf)
{
	...
	e->sections = vector_alloc();
	e->symbols  = vector_alloc();
...
int elf32_add_sym(elf_context_t* elf, const elf_sym_t* sym, const char* sh_name)
{
	...
	if (sym->name)
		xsym->name = string_cstr(sym->name);
	else
		xsym->name = NULL;
	...
	int ret = vector_add(vec, xsym);
	if (ret < 0) {
		string_free(xsym->name);
		free(xsym);
		return ret;
	}
...
int elf32_add_section(elf_context_t* elf, const elf_section_t* section)
{
	...
	s->name = string_cstr(section->name);
	if (!s->name) {
		free(s);
		return -ENOMEM;
	}
...
int elf32_read_shstrtab(elf_context_t* elf)
{
	...
	if (!e->sh_shstrtab_data) {
		e->sh_shstrtab_data = string_alloc();
		if (!e->sh_shstrtab_data)
			return -ENOMEM;
	}
...
static int __elf_read_section_data(elf_context_t* elf, elf_section_t* s)
{
...
static int __elf_read_section_by_index(elf_context_t* elf, elf_section_t** psection, const int index)
{
	elf_native_t* e = elf->priv;
	...
	s->index = index;
	s->name  = string_cstr(e->sh_shstrtab_data->data + s->sh.sh_name);
	if (!s->name) {
		free(s);
		return -1;
	}

	ret = vector_add(e->sections, s);
	if (ret < 0) {
		string_free(s->name);
		free(s);
		return -1;
...
static int __elf_read_section(elf_context_t* elf, elf_section_t** psection, const char* name)
{
	elf_native_t* e = elf->priv;
	...
	for (i = 0; i < e->sections->size; i++) {
		...
		if (!string_cmp_cstr(s->name, name)) {
...
int elf32_read_syms(elf_context_t* elf, vector_t* syms, const char* sh_name)
{
	...
	for (i  = 0; i < symtab->data_len; i += sizeof(Elf32_Sym)) {
		...
		esym = calloc(1, sizeof(elf_sym_t));
		if (!esym)
			return -ENOMEM;
		...
		if (vector_add(syms, esym) < 0) {
			free(esym);
			return -ENOMEM;
		}
	}

	logd("1: j: %d, syms->size: %d, %ld\n", j, syms->size, symtab->data_len / sizeof(Elf32_Sym));
...
int elf32_add_dyn_need(elf_context_t* elf, const char* soname)
{
	...
	if (!e->dyn_needs) {
		e->dyn_needs = vector_alloc();
		if (!e->dyn_needs)
			return -ENOMEM;
	}

	string_t* s = string_cstr(soname);
	if (!s)
		return -ENOMEM;

	if (vector_add(e->dyn_needs, s) < 0) {
		string_free(s);
		return -ENOMEM;
	}
...
int elf32_read_relas(elf_context_t* elf, vector_t* relas, const char* sh_name)
{
	...
	if (SHT_RELA == sh_rela->sh.sh_type) {
		...
		for (i   = 0; i < sh_rela->data_len; i += sizeof(Elf32_Rela)) {
			...
			erela = calloc(1, sizeof(elf_rela_t));
			if (!erela)
				return -ENOMEM;
...
int elf32_read_section(elf_context_t* elf, elf_section_t** psection, const char* name)
{
	...
	s2 = calloc(1, sizeof(elf_section_t));
	if (!s2)
		return -ENOMEM;
...
int elf32_find_sym(elf_sym_t** psym, Elf32_Rela* rela, vector_t* symbols)
{
	...
	if (0 == sym->sym.st_shndx) {
		...
		if (0 == n) {
			loge("global symbol: %s not found\n", sym->name->data);
			return -1;

		} else if (n > 1) {
			loge("tow global symbol: %s\n", sym->name->data);
			return -1;
		}
...
void elf32_process_syms(elf_native_t* e, uint32_t cs_index)
{
	...
	for (i  = e->symbols->size - 1; i >= 0; i--) {
		...
		assert(0 == vector_del(e->symbols, sym));
