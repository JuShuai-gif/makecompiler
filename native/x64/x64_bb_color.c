...
int x64_init_bb_colors(basic_block_t* bb)
{
	...
	for (i = 0; i < bb->dn_colors_entry->size; i++) {
		...
		if (0 == bb->index && dn->rabi) {
			...
			int ret = vector_add_unique(r->dag_nodes, dn);
			if (ret < 0)
				return ret;
		...
		if (vector_find(bb->dn_loads, dn)) {
			variable_t* v = dn->var;
			if (v->w)
				logd("v_%d_%d/%s, ", v->w->line, v->w->pos, v->w->text->data);
			else
				logd("v_%#lx, ", 0xffff & (uintptr_t)v);
...
int x64_load_bb_colors(basic_block_t* bb, bb_group_t* bbg, function_t* f)
{
	...
	for (i = 0; i < bb->dn_colors_entry->size; i++) {
		...
		if (color != dn->color && color > 0) {
			register_t* r = x64_find_register_color(color);
