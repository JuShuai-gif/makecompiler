...
static int _unary_assign_sib(x64_sib_t* sib, int size, _3ac_code_t* c, function_t* f, int OpCode_type)
{
	x64_OpCode_t*   OpCode;
	instruction_t*  inst;

	OpCode = x64_find_OpCode(OpCode_type, size, size, X64_E);
	if (!OpCode) {
		loge("\n");
		return -EINVAL;
	}
...
int x64_unary_assign_dereference(native_t* ctx, _3ac_code_t* c, int OpCode_type)
{
...
int x64_unary_assign_pointer(native_t* ctx, _3ac_code_t* c, int OpCode_type)
{
...
int x64_unary_assign_array_index(native_t* ctx, _3ac_code_t* c, int OpCode_type)
{
