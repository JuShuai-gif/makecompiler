...
static int _shift_count(dag_node_t* count, _3ac_code_t* c, function_t* f)
{
	register_t*     rc   = NULL;
	register_t*     cl   = x64_find_register("cl");
	...
	if (count->color > 0) {
		...
		if (!X64_COLOR_CONFLICT(rc->color, cl->color)) {
			...
			mov  = x64_find_OpCode(X64_MOV, 1, 1, X64_G2E);
		...
		mov  = x64_find_OpCode(X64_MOV, 1, 1, X64_E2G);
...
static int _x64_shift(native_t* ctx, _3ac_code_t* c, dag_node_t* dst, dag_node_t* count, int OpCode_type)
{
	...
	if (dst->color > 0) {
		...
		if (0 != count->color) {
			shift = x64_find_OpCode(OpCode_type, 1, dst->var->size, X64_G2E);
			inst  = x64_make_inst_E(shift, rd);
			X64_INST_ADD_CHECK(c->instructions, inst);
		} else {
			shift = x64_find_OpCode(OpCode_type, 1, dst->var->size, X64_I2E);
...
int x64_shift(native_t* ctx, _3ac_code_t* c, int OpCode_type)
{
	...
	if (X64_SHR == OpCode_type
			&& type_is_signed(dst->dag_node->var->type))
		OpCode_type = X64_SAR;
...
int x64_shift_assign(native_t* ctx, _3ac_code_t* c, int OpCode_type)
{
	...
	x64_context_t* x64   = ctx->priv;
	function_t*    f     = x64->f;
	_3ac_operand_t* count = c->srcs->data[0];
	_3ac_operand_t* dst   = c->dsts->data[0];
