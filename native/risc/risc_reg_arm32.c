...
#define RISC_REG_FP 11
#define RISC_REG_SP 13
#define RISC_REG_LR 14
#define RISC_REG_PC 15

register_t	arm32_registers[] = {
...
static int arm32_variable_size(variable_t* v)
{
...
register_t* arm32_find_register(const char* name)
{
	...
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
		...
		register_t*	r = &(arm32_registers[i]);
...
register_t* arm32_find_register_type_id_bytes(uint32_t type, uint32_t id, int bytes)
{
	int i;
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
...
register_t* arm32_find_register_color(intptr_t color)
{
	int i;
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
...
register_t* arm32_find_register_color_bytes(intptr_t color, int bytes)
{
	int i;
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
...
vector_t* arm32_register_colors()
{
	...
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
		...
		int ret = vector_add(colors, (void*)r->color);
		if (ret < 0) {
			vector_free(colors);
			return NULL;
		}
...
int arm32_reg_cached_vars(register_t* r)
{
	int nb_vars = 0;
	int i;
	...
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
		...
		register_t*	r2 = &(arm32_registers[i]);

		if (RISC_REG_SP == r2->id
         || RISC_REG_FP == r2->id
         || RISC_REG_LR == r2->id
		 || RISC_REG_X12 == r2->id)
			continue;
...
static register_t* risc_reg_cached_min_vars(register_t** regs, int nb_regs)
{
	register_t* r_min = NULL;
...
int arm32_caller_save_regs(_3ac_code_t* c, function_t* f, uint32_t* regs, int nb_regs, int stack_size, register_t** saved_regs)
{
	int i;
	int j;
	instruction_t* inst;
	register_t*    r;
	register_t*    r2;
	register_t*    sp = arm32_find_register("sp");
	...
	if (stack_size > 0) {
		for (j = 0; j < k / 2; j++) {
			...
			i  = k - 1 - j;
			XCHG(saved_regs[i], saved_regs[j]);
		}
	}
...
int arm32_pop_regs(_3ac_code_t* c, function_t* f, register_t** regs, int nb_regs, register_t** updated_regs, int nb_updated)
{
	int i;
	int j;
...
int arm32_registers_reset()
{
	...
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
		...
		while (j < r->dag_nodes->size) {
			dag_node_t* dn = r->dag_nodes->data[j];
			...
			int ret = vector_del(r->dag_nodes, dn);
			if (ret < 0) {
				loge("\n");
				return ret;
			}
...
int arm32_overflow_reg(register_t* r, _3ac_code_t* c, function_t* f)
{
	int i;
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
...
int arm32_overflow_reg2(register_t* r, dag_node_t* dn, _3ac_code_t* c, function_t* f)
{
	register_t*	r2;
	dag_node_t*     dn2;
...
int arm32_overflow_reg3(register_t* r, dag_node_t* dn, _3ac_code_t* c, function_t* f)
{
	register_t*	r2;
	dn_status_t*    ds2;
	dag_node_t*     dn2;
	...
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
		...
		for (j  = 0; j < r2->dag_nodes->size; ) {
			...
			ds2 = vector_find_cmp(c->active_vars, dn2, dn_status_cmp);
			if (!ds2) {
				j++;
				continue;
			}
...
int arm32_reg_used(register_t* r, dag_node_t* dn)
{
	register_t*	r2;
	dag_node_t*     dn2;
...
register_t* arm32_select_overflowed_reg(dag_node_t* dn, _3ac_code_t* c, int is_float)
{
	vector_t*       neighbors = NULL;
	graph_node_t*   gn        = NULL;
	register_t*     free_regs[sizeof(arm32_registers) / sizeof(arm32_registers[0])];
	...
	if (dn) {
		is_float =   variable_float(dn->var);
	...
	for (i = 0; i < sizeof(arm32_registers) / sizeof(arm32_registers[0]); i++) {
		...
		for (j = 0; j < neighbors->size; j++) {
			...
			graph_node_t* neighbor = neighbors->data[j];
...
static void arm32_argv_rabi(function_t* f)
{
	variable_t* v;
	...
	for (i = 0; i < f->argv->size; i++) {
		...
		if (is_float) {
			...
			if (f->args_int   < f->rops->ABI_NB) {
				...
				logd("i: %d, is_float: %d, size: %d, rid: %d, %s\n", i, is_float, size, rid, v->rabi->name);
				continue;
			}
		}
...
void arm32_call_rabi(_3ac_code_t* c, function_t* f, int* p_nints, int* p_nfloats, int* p_ndoubles)
{
...
void arm32_call_rabi_varg(_3ac_code_t* c, function_t* f)
{
	_3ac_operand_t* src = NULL;
	dag_node_t*    dn  = NULL;
...
int arm32_push_callee_regs(_3ac_code_t* c, function_t* f)
{
	instruction_t* inst;
	register_t*    r2;
	register_t*    r;
...
int arm32_pop_callee_regs(_3ac_code_t* c, function_t* f)
{
	instruction_t* inst;
	register_t*    r2;
	register_t*    r;
