...
#define RISC_REG_FP 29
#define RISC_REG_LR 30
#define RISC_REG_SP 31

register_t	arm64_registers[] = {
...
static int arm64_variable_size(variable_t* v)
{
...
register_t* arm64_find_register(const char* name)
{
	int i;
	for (i = 0; i < sizeof(arm64_registers) / sizeof(arm64_registers[0]); i++) {

		register_t*	r = &(arm64_registers[i]);
...
register_t* arm64_find_register_type_id_bytes(uint32_t type, uint32_t id, int bytes)
{
	int i;
	for (i = 0; i < sizeof(arm64_registers) / sizeof(arm64_registers[0]); i++) {
...
register_t* arm64_find_register_color(intptr_t color)
{
	int i;
	for (i = 0; i < sizeof(arm64_registers) / sizeof(arm64_registers[0]); i++) {
...
register_t* arm64_find_register_color_bytes(intptr_t color, int bytes)
{
	int i;
	for (i = 0; i < sizeof(arm64_registers) / sizeof(arm64_registers[0]); i++) {
...
vector_t* arm64_register_colors()
{
...
int arm64_reg_cached_vars(register_t* r)
{
	int nb_vars = 0;
	int i;
	...
	for (i = 0; i < sizeof(arm64_registers) / sizeof(arm64_registers[0]); i++) {
		...
		register_t*	r2 = &(arm64_registers[i]);
...
int arm64_caller_save_regs(_3ac_code_t* c, function_t* f, uint32_t* regs, int nb_regs, int stack_size, register_t** saved_regs)
{
	int i;
	int j;
	register_t* r;
	register_t* r2;
	instruction_t*    inst;
	register_t* sp   = arm64_find_register("sp");
...
int arm64_pop_regs(_3ac_code_t* c, function_t* f, register_t** regs, int nb_regs, register_t** updated_regs, int nb_updated)
{
	int i;
	int j;
...
int arm64_overflow_reg(register_t* r, _3ac_code_t* c, function_t* f)
{
	int i;
...
int arm64_overflow_reg2(register_t* r, dag_node_t* dn, _3ac_code_t* c, function_t* f)
{
	register_t*	r2;
	dag_node_t* dn2;
...
int arm64_overflow_reg3(register_t* r, dag_node_t* dn, _3ac_code_t* c, function_t* f)
{
	register_t*	r2;
	dn_status_t*    ds2;
	dag_node_t*     dn2;
...
int arm64_reg_used(register_t* r, dag_node_t* dn)
{
	register_t*	r2;
	dag_node_t*     dn2;
...
register_t* arm64_select_overflowed_reg(dag_node_t* dn, _3ac_code_t* c, int is_float)
{
	vector_t*       neighbors = NULL;
	graph_node_t*   gn        = NULL;

	register_t* free_regs[sizeof(arm64_registers) / sizeof(arm64_registers[0])];
...
static void arm64_argv_rabi(function_t* f)
{
	variable_t* v;
...
void arm64_call_rabi(_3ac_code_t* c, function_t* f, int* p_nints, int* p_nfloats, int* p_ndoubles)
{
...
int arm64_push_callee_regs(_3ac_code_t* c, function_t* f)
{
	instruction_t* inst;
	register_t*    r;
...
int arm64_pop_callee_regs(_3ac_code_t* c, function_t* f)
{
	instruction_t* inst;
	register_t*    r;
